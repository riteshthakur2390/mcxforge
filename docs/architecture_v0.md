...
├── ml/
    ├── __init__.py
    ├── features.py          ← Single source of truth for all feature extraction
    ├── model.py             ← SignalForgeEnsemble class (train + predict + save + load)
    ├── saved_models/
    │   └── README.md        ← Explains what goes here, model format, when to train
    └── training/
        ├── __init__.py
        ├── trainer.py       ← Full pipeline: load journal → features → train → evaluate → save
        └── evaluate.py      ← Post-training analysis: feature importance, threshold analysis
├── backtesting/
